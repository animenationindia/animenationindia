...
const TopAnime = () => {
  ...
  useEffect(() => {
    const getTopAnime = async () => {
      setLoadingTop(true);
      const data = await fetchJikan('ani_top_anime_3', 'https://api.jikan.moe/v4/top/anime?limit=3');
      if (data && data.data) {
        setTopAnime(removeDuplicates(data.data));
      }
      setLoadingTop(false);
    };
    getTopAnime();
  ...
  useEffect(() => {
    const getAllAnime = async () => {
      setLoadingAll(true);
      const cacheKey = `ani_all_anime_page_${currentPage}`;
      const data = await fetchJikan(cacheKey, `https://api.jikan.moe/v4/anime?page=${currentPage}&limit=24`, true); 
      
      if (data && data.data) {
        setAllAnime(removeDuplicates(data.data));
        if (data.pagination) {
      ...
      if(currentPage > 1) {
         document.getElementById('all-anime-section').scrollIntoView({ behavior: 'smooth' });
      }
    };
    getAllAnime();
  }, [currentPage]);
  ...
  // ===============================================================
  // 🔥 UPDATED: PERSONAL DATABASE SAVE LOGIC 🔥
  // ===============================================================
  const addToWatchlist = async (e, anime) => {
    ...
    try {
        const response = await fetch('http://localhost:5000/api/watchlist', {
            ...
            body: JSON.stringify({ 
                anime: anime,
                userId: userId // Send userId to make it personal
            }) 
        });
  ...
  return (
    <div className="page-wrap" style={{ paddingTop: '8rem', maxWidth: '1200px', margin: '0 auto', paddingBottom: '5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px', marginBottom: '3rem', padding: '0 20px' }}>
          ...
          <div style={{ position: 'relative', width: '100%', maxWidth: '350px', zIndex: 50 }}>
            <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '99px', padding: '5px 15px', alignItems: 'center', gap: '10px' }}>
              ...
              <input 
                type="text" placeholder="Search Anime..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              ...
              <div style={{ position: 'absolute', top: '110%', left: 0, right: 0, background: '#121326', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '15px', boxShadow: '0 20px 40px rgba(0,0,0,0.8)', overflow: 'hidden' }}>
                ...
                : searchResults.length > 0 ? searchResults.map((anime, index) => (
                    <Link to={`/anime/${anime.mal_id}`} key={index} style={{ display: 'flex', gap: '15px', padding: '15px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer', alignItems: 'center', textDecoration: 'none' }}>
                      <img src={anime.images?.webp?.large_image_url} alt="poster" style={{ width: '40px', height: '60px', borderRadius: '4px', objectFit: 'cover' }} />
                      <div>
                        <h4 style={{ color: '#fff', fontSize: '0.9rem', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{anime.title_english || anime.title}</h4>
                        <span style={{ fontSize: '0.75rem', color: 'var(--accent)', marginTop: '5px', display: 'inline-block' }}>{anime.type || 'TV'} • {anime.score || 'N/A'} <i className="fas fa-star"></i></span>
                      </div>
                    </Link>
                  )) : <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-muted)' }}>No anime found.</div>}
              </div>
            )}
          </div>
      </div>

      {/* ================= TOP 3 ANIME (Horizontal Cards) ================= */}
          ...
          {loadingTop ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  {topAnime.map((anime, index) => (
                      <Link to={`/anime/${anime.mal_id}`} key={anime.mal_id} style={{ display: 'flex', background: 'var(--bg-elevated)', borderRadius: '20px', padding: '20px', gap: '20px', alignItems: 'center', textDecoration: 'none', border: '1px solid rgba(255,255,255,0.05)' }} className="hover-scale">
                          <div style={{ fontSize: '2rem', fontWeight: 'bold', color: 'var(--text-muted)', width: '50px', textAlign: 'center' }}>#{index + 1}</div>
                          <img src={anime.images?.webp?.large_image_url} alt={anime.title} style={{ width: '80px', height: '120px', objectFit: 'cover', borderRadius: '10px', boxShadow: '0 10px 20px rgba(0,0,0,0.5)' }} />
                          <div style={{ flex: 1 }}>
                              <h3 style={{ color: '#fff', fontSize: '1.4rem', marginBottom: '10px' }}>{anime.title_english || anime.title}</h3>
                              ...
                              <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                                  {anime.type || 'TV'} • {anime.aired?.prop?.from?.year || 'Unknown'} • {anime.episodes ? `${anime.episodes} Eps` : 'Ongoing'}
                              </div>
                          </div>
                          <div style={{ textAlign: 'right', paddingRight: '20px' }}>
                              <div style={{ fontSize: '1.8rem', color: '#fff', fontWeight: 'bold' }}><i className="fas fa-star" style={{ color: 'var(--accent)', fontSize: '1.2rem' }}></i> {anime.score}</div>
                              {/* 🔥 WATCHLIST BUTTON 🔥 */}
                              <button onClick={(e) => addToWatchlist(e, anime)} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', padding: '6px 15px', borderRadius: '99px', cursor: 'pointer', fontSize: '0.8rem', marginTop: '10px', display: 'inline-flex', alignItems: 'center', gap: '5px', zIndex: 10, position: 'relative' }}>
                                  + Bookmark
                              </button>
                          </div>
                      </Link>
      ...
      {/* ================= ALL ANIME SECTION ================= */}
      <div id="all-anime-section" style={{ padding: '0 20px', paddingTop: '20px' }}>
          <h2 style={{ color: '#fff', fontSize: '1.8rem', borderLeft: '4px solid var(--accent)', paddingLeft: '15px', marginBottom: '30px' }}>All Anime Collection</h2>
          
          {loadingAll ? (
              <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '100px 0' }}><i className="fas fa-spinner fa-spin fa-3x"></i><p style={{marginTop: '20px'}}>Loading Anime Page {currentPage}...</p></div>
          ) : (
              <>
                  <div className="grid-wrap" style={{ display: 'grid', gap: '20px', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', marginBottom: '40px' }}>
                      {allAnime.map((anime) => (
                          <div key={`all-${anime.mal_id}`} className="anime-card hover-scale" style={{ background: 'var(--bg-elevated)', borderRadius: '16px', overflow: 'hidden', position: 'relative', cursor: 'pointer' }} onClick={() => navigate(`/anime/${anime.mal_id}`)}>
                              <div style={{ position: 'relative' }}>
                                  <img src={anime.images?.webp?.large_image_url} alt={anime.title} style={{ width: '100%', height: '280px', objectFit: 'cover' }} />
                              ...
                              <div style={{ padding: '15px' }}>
                                  <div style={{ color: '#fff', fontWeight: 'bold', fontSize: '1rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{anime.title_english || anime.title}</div>
                                  <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '5px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{anime.genres?.map(g=>g.name).join(', ') || 'Anime'}</div>
                              </div>
                              {/* 🔥 FLOATING WATCHLIST ICON BUTTON 🔥 */}
                              <button onClick={(e) => addToWatchlist(e, anime)} style={{ position: 'absolute', bottom: '15px', right: '15px', background: 'var(--primary)', border: 'none', color: '#fff', width: '35px', height: '35px', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10, boxShadow: '0 4px 10px rgba(0,0,0,0.5)' }}>
